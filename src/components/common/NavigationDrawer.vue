...
const onTouchStart = (event) => { startX.value = event.changedTouches[0]?.clientX ?? 0 }
const onTouchEnd = (event) => {
  const distance = (event.changedTouches[0]?.clientX ?? startX.value) - startX.value
  if (distance < -48) close()
}

const onEdgeTouchStart = (event) => {
  if (!isOpen.value && (event.touches[0]?.clientX ?? 999) < 28) startX.value = event.touches[0].clientX
}
const onEdgeTouchEnd = (event) => {
  const endX = event.changedTouches[0]?.clientX ?? 0
  if (!isOpen.value && startX.value < 28 && endX - startX.value > 56) open()
}
